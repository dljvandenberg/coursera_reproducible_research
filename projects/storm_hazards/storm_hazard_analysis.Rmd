...
Furthermore we load an the official set of event types, extracted from the National Weather Service Storm Data [Documentation](https://d396qusza40orc.cloudfront.net/repdata%2Fpeer2_doc%2Fpd01016005curr.pdf). Its contents are show below.
```{r}
df.eventtypes <- read.csv("event_types.txt", stringsAsFactors=FALSE)
as.list(df.eventtypes)
```


## Data cleaning & selecting relevant subset

The raw data set consists of close to a million measurements of 37 variables.
```{r}
dim(df.stormdata)
```
...
summary(df.stormdata.subset.alltime[df.stormdata.subset.alltime$PROPDMGEXP %in% c("-", "?", "+", "h", "H", "0", "1", "2"),])
# => Strange values for PROPDMGEXP, all between 1993 and 1995
sort(unique(df.stormdata.subset.alltime$CROPDMGEXP))
summary(df.stormdata.subset.alltime[df.stormdata.subset.alltime$CROPDMGEXP %in% c("?", "0", "2"),])
...
summary(df.stormdata.subset.alltime[df.stormdata.subset.alltime$PROPDMGEXP %in% c("B"),])
# => Is this entry correct?
# 605953   1/1/2006 0:00:00                      FLOOD          0        0  115.00          B   32.50          M
summary(df.stormdata.subset.alltime[df.stormdata.subset.alltime$CROPDMGEXP %in% c("B"),])
```
...
## Weather impact on Population Health
